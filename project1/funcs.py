...
class functions:
    ...
    def bootstrap(n, A_train, A_test, f_train, f_test, model):
        ...
        score_train = np.zeros(n)
        fte_aval = np.zeros((len(f_test),n))
        ftr_aval = np.zeros((len(f_train),n))
        for j in range(0,n):
            ...
            ftr_aval[:,j] = ATr_res @ beta + f_scale
            
            score_test[j] = np.mean((f_test - fte_aval[:,j])**2)
            score_train[j] = np.mean((f_train - ftr_aval[:,j])**2)
        return np.mean(score_train), np.mean(score_test), ftr_aval, fte_aval
