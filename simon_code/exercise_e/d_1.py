...
#Ridge Stuff
lambdas = np.power(10.0,2-np.arange(8))
...
for i in range(1,max_order+1):
    ...
    for ridge_par in lambdas: 
        ...
        f_approx_test[:,0] = A_test @ beta + z_scale
        for j in range(n_bootstrap+1):
            ...
            f_approx_test[:,j] = A_test @ beta + z_scale
...
sns.heatmap(MSE_KFOLD.T, annot=True, ax=ax, cmap="viridis", cbar_kws={'label': 'Accuracy'})
ax.set_title("Test Accuracy KFOLD 1")
ax.set_ylabel("order")
ax.set_xlabel("log$(_{10}(\lambda))$")
ax.set_xticklabels(np.log10(lambdas))
...
fig, ax = plt.subplots(figsize=(10, 5))
sns.heatmap(MSE_bs.T, annot=True, ax=ax, cmap="viridis", cbar_kws={'label': 'Accuracy'})
